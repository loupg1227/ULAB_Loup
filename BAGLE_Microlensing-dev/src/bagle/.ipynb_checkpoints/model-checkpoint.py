...
from src.bagle import orbits as orbits
from src.bagle import parallax
from abc import ABC
...
class binary_shrinking_orbit(PSBL_PhotAstromParam4):
    """
    Point source binary lens.
    It has 3 more parameters than PSPL (additional mass term, separation,
    and angle of approach). Note that this is a non-STATIC binary lens, i.e.
    ...
    def __init__(self, t0_com, u0_amp_com, tE, thetaE, piS,
                 piE_E, piE_N, xS0_E, xS0_N, omega, big_omega, i, e, tp, sep, da_dt, arat, muS_E, muS_N,
                 q, alpha,
                 b_sff, mag_src,
                 raL=None, decL=None, obsLocation='earth', root_tol=1e-8):
        super().__init__(t0_com, u0_amp_com, tE, thetaE, piS,
        ...
        self.tp = tp
         ...
         # Calculate period, and semi-major axes
        self.sep = sep #mas
        self.a = self.arat * self.sep #mas CHANGE TO INCORPORATE BINARY ORBIT
        self.aleph_sec = (self.mLp/(self.mLp+self.mLs))*self.a #mas SECONDARY SEMI MAJOR AXIS
        self.aleph = self.a - self.aleph_sec #mas PRIMARY SEMI MAJOR AXIS
        self.al_AU = self.dL * (self.a *1e-3) * units.AU
        mL = self.mL * units.Msun
        p = (2 * np.pi * np.sqrt(self.al_AU ** 3 / (const.G * mL))).to('day') # CHANGE TO INCORPORATE BINARY ORBIT
        self.p = p.value  # Period in Days
